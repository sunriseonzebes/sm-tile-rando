...
class TestTRSimpleBoxRoomGenerator(unittest.TestCase):
    ...
    def test_generate_door_attach_points(self):
        ...
        test_gen._width = 1
        test_gen._height = 1
        expected_result = [tr_door_attach_point.TRDoorAttachPoint(0, 0, [DoorExitDirection.UP,
                                                                         DoorExitDirection.DOWN,
        ...
        expected_result = [tr_door_attach_point.TRDoorAttachPoint(0, 0, [DoorExitDirection.UP,
                                                                         DoorExitDirection.LEFT]),
                           tr_door_attach_point.TRDoorAttachPoint(0, 1, [DoorExitDirection.DOWN,
                                                                         DoorExitDirection.LEFT]),
                           tr_door_attach_point.TRDoorAttachPoint(1, 0, [DoorExitDirection.UP,
                                                                         DoorExitDirection.RIGHT]),
                           tr_door_attach_point.TRDoorAttachPoint(1, 1, [DoorExitDirection.DOWN,
                                                                         DoorExitDirection.RIGHT])]
        actual_result = test_gen.generate_door_attach_points()
        for i in range(len(expected_result)):
            self.assertEqual(expected_result[i].h_screen,
        ...
        expected_result = [tr_door_attach_point.TRDoorAttachPoint(0, 0, [DoorExitDirection.UP,
                                                                         DoorExitDirection.LEFT]),
                           tr_door_attach_point.TRDoorAttachPoint(0, 1, [DoorExitDirection.LEFT]),
                           tr_door_attach_point.TRDoorAttachPoint(0, 2, [DoorExitDirection.DOWN,
                                                                         DoorExitDirection.LEFT]),
                           tr_door_attach_point.TRDoorAttachPoint(1, 0, [DoorExitDirection.UP]),
                           tr_door_attach_point.TRDoorAttachPoint(1, 2, [DoorExitDirection.DOWN]),
                           tr_door_attach_point.TRDoorAttachPoint(2, 0, [DoorExitDirection.UP,
                                                                         DoorExitDirection.RIGHT]),
                           tr_door_attach_point.TRDoorAttachPoint(2, 1, [DoorExitDirection.RIGHT]),
                           tr_door_attach_point.TRDoorAttachPoint(2, 2, [DoorExitDirection.DOWN,
                                                                         DoorExitDirection.RIGHT])
                           ]
        actual_result = test_gen.generate_door_attach_points()
